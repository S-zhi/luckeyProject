...
const storageCodeBackend = "backend"
const storageCodeBaidu = "baidu_netdisk"
...
func normalizeStorageServerCodeField(raw string) string {
	value := strings.TrimSpace(raw)
	servers := make([]string, 0, 2)
	if value != "" {
		var arr []string
	...
	normalized := normalizeStorageServerCodeList(servers)
	if len(normalized) == 0 {
		normalized = []string{storageCodeBackend}
	}
	bytes, err := json.Marshal(normalized)
	if err != nil {
		return "[\"backend\"]"
...
func normalizeStorageServerCodeList(servers []string) []string {
	seen := make(map[string]struct{}, len(servers))
	result := make([]string, 0, len(servers))
	for _, server := range servers {
		label, ok := toStorageCode(server)
		if !ok {
			continue
		}
		if _, exists := seen[label]; exists {
...
func toStorageCode(raw string) (string, bool) {
	value := strings.TrimSpace(raw)
	if value == "" {
		return "", false
	}
	...
	switch strings.ToLower(value) {
	case StorageTargetBaiduNetdisk, "baidu", "baidu-pan", "baidu_pan", "baidupan", "pan.baidu", "百度网盘":
		return storageCodeBaidu, true
	case storageCodeBackend, "local", "本地存储":
		return storageCodeBackend, true
	default:
		return value, true
	}
}
